...
const Board = (props) => {
	...
	const [memberFilter, setMemberFilter] = useState(null); // stores selected member userId or null
	...
	useEffect(() => {
		document.title = title + ' | Tablero de Trabajo';
	}, [title]);
						...
						{(provided, snapshot) => {
							return (
								<style.ListContainer {...provided.droppableProps} ref={provided.innerRef}>
									{!loading &&
										(Array.isArray(allLists) ? allLists.filter(Boolean) : []).map((list, index) => {
											return (
												<List
													searchString={searchString}
													memberFilter={memberFilter}
