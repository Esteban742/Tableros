									...
									return (
										...
										>
											<CardWrapper dock={clickFooter}>
												{props.info.cards
													.filter(card => {
														const matchesText = props.searchString ? card.title.toLowerCase().includes(props.searchString.toLowerCase()) : true;
														const matchesMember = props.memberFilter ? (Array.isArray(card.members) && card.members.some(m => (m.user?.toString?.() || m.user) === props.memberFilter)) : true;
