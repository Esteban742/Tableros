...
const getAll = async (boardId, callback) => {
	try {
		...
		let responseObject = board.lists
			.map((listId) => lists.find((listObject) => listObject._id.toString() === listId.toString()))
			.filter((x) => !!x);
	...
	try {
		...
		if (sourceId !== destinationId)
			card.activities.unshift({
				text: `movio esta tarjeta de ${sourceList.title} a ${destinationList.title}`,
				userName: user.name,
				color: user.color,
			});
