...
const verifyTokenWrapper = async (req, res, next) => {
  try {
    ...
    // ✅ Asegurar consistencia en el formato del usuario
    req.user = { 
      id: user._id.toString(),           // ✅ id como string para comparaciones
      _id: user._id,                     // ✅ _id original ObjectId
      email: user.email, 
      name: user.name,
      boards: user.boards || []
    };
    ...
    console.log(`[TokenMiddleware] 📝 req.user.id: ${req.user.id}`);
    console.log(`[TokenMiddleware] 📝 req.user.boards: ${req.user.boards.length} boards`);
